...
# Import conditional SEC filings fetcher
from sec_filings_fetcher import fetch_sec_filings, fetch_sec_company_info
...
class DanDanDonkeyBot(ForecastBot):
    ...
    Wikipedia, WHO GHO, ACLED, GDELT media trends, SEC filings (conditional).
    """
    ...
    async def run_research(self, question: MetaculusQuestion) -> str:
        async with self._concurrency_limiter:
            ...
            reasoning = await self.get_llm("researcher", "llm").invoke(research_prompt)
    ...
    async def _run_forecast_on_multiple_choice(
        ...
        prompt = clean_indents(
            ...
            Before answering you write:
            (a) The time left until resolution.
            (b) The status quo outcome if nothing changed.
            (c) The OUTSIDE VIEW -- what % of similar situations resolve to each outcome?
            (d) The INSIDE VIEW adjustment -- what specific current evidence shifts probabilities?
            (e) Key scenarios and outcomes that could arise.
            (f) If prediction market or community forecasts are available, note them and whether you agree.

            Rationale guidelines:
            - Anchor on the outside view base rates, then adjust with inside view evidence.
            - Use your research to inform each option's likelihood.
            - All probabilities must sum to 100%.
            {self._get_conditional_disclaimer_if_necessary(question)}

            The last thing you write is your final answer in the exact format:
            "Option_1_Name: ZZ%
            Option_2_Name: ZZ%
    ...
    async def _multiple_choice_prompt_to_forecast(
        ...
        reasoning = await self.get_llm("default", "llm").invoke(prompt)
        logger.info(f"Reasoning for {question.page_url}: {reasoning}")
        parsed_prediction: PredictedOptionList = await structure_output(
            reasoning, PredictedOptionList,
            model=self.get_llm("parser", "llm"),
            additional_instructions=f"Parse into {len(question.options)} options: {[opt.name for opt in question.options]}",
            num_validation_samples=self._structure_output_validation_samples,
        )
        logger.info(f"Forecasted {question.page_url}: {parsed_prediction.option_predictions}")
        return ReasonedPrediction(prediction_value=parsed_prediction, reasoning=reasoning)
    ...
    async def _run_forecast_on_numeric(
        ...
        prompt = clean_indents(
            ...
            Before answering:
            (a) Time left until resolution. (b) OUTSIDE VIEW. (c) INSIDE VIEW.
            (d) Expert expectations for this quantity. (e) Low scenario. (f) High scenario.

            {self._get_conditional_disclaimer_if_necessary(question)}
            Set a wide confidence interval.

            Final answer (in {question.unit_of_measure}):
            "Percentile 10: X
            Percentile 20: X
            Percentile 40: X
            Percentile 60: X
    ...
    async def _numeric_prompt_to_forecast(self, question: NumericQuestion, prompt: str) -> ReasonedPrediction[NumericDistribution]:
        ...
        parsing_instructions = clean_indents(
            ...
            Range: {question.lower_bound} to {question.upper_bound} {question.unit_of_measure}.
            If percentiles not given, indicate that and estimate from ranges.
            """
        )
        numeric_percentile_list: list[Percentile] = await structure_output(
            reasoning, list[Percentile],
            model=self.get_llm("parser", "llm"),
            additional_instructions=parsing_instructions,
            num_validation_samples=self._structure_output_validation_samples,
        )
        prediction = NumericDistribution.from_question(numeric_percentile_list, question)
